namespace El_juguete_de_Elon
{
    /*
        Consigna: 
            En este ejercicio jugarás con un coche controlado a distancia y 
    finalmente has ahorrado suficiente dinero para comprarlo.

            Los coches arrancan con las baterías llenas (100%). 
    Cada vez que conduces el coche con el mando a distancia, recorre 20 metros y
    agota el uno por ciento de la batería.

            El automóvil controlado a distancia tiene una elegante pantalla LED que muestra dos bits 
    de información:
            ...
            Si la batería está al 0%, ya no podrá conducir el automóvil y la pantalla de 
    la batería mostrará "Batería vacía".

            Tiene seis tareas, cada una de las cuales funcionará con instancias de automóviles 
    controladas remotamente.

            Tarea 1: Compra un coche nuevo a control remoto
                Implemente el método ( estático ) RemoteControlCar.Comprar()para devolver una 
    nueva instancia de automóvil controlado a distancia.
                    RemoteControlCar car = RemoteControlCar.Comprar();

            Tarea 2: Mostrar la distancia recorrida
                Implemente el método RemoteControlCar.MostrarDistancia() para devolver la 
    distancia como se muestra en la pantalla LED.
                    var car = RemoteControlCar.Comprar();
                    car.MostrarDistancia();
                    // => "Distancia conducida: 0 metros"
    
            Tarea 3: Mostrar el porcentaje de batería
                Implemente el método RemoteControlCar.MostrarBateria() para devolver el 
    porcentaje de batería como se muestra en la pantalla LED.
            ...
            Tarea 4: Actualizar el número de metros recorridos al conducir
                Implementar el método RemoteControlCar.Conducir() que actualiza la cantidad 
    de metros recorridos.
            ...
            Tarea 6: Evite conducir cuando la batería esté agotada
                Actualice el método RemoteControlCar.Conducir() para no aumentar la distancia
    recorrida ni disminuir el porcentaje de batería cuando la batería está agotada (al 0%)
                    var car = RemoteControlCar.Comprar();
    ...
    class RemoteControlCar
    {
        ...
        public string MostrarDistancia()
        {
            if (bateria <= 0)
            {
                return "Distancia conducida: 2000 metros";
            }
